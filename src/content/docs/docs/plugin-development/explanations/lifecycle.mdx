...
For example, the `index.ts` file of the [Hardhat 3 plugin template](https://github.com/NomicFoundation/hardhat3-plugin-template/blob/main/packages/plugin/src/index.ts) looks like this:
...
The task action, either the inline callback you pass to `setInlineAction()`, or the `default` export of the module loaded via `setAction()`, can optionally return a `Result` value. If the action returns a result with `success: false`, the CLI sets the process exit code to 1.

To learn more, see the [Returning a result](/docs/guides/writing-tasks#returning-a-result) section of the Writing Hardhat tasks guide.
