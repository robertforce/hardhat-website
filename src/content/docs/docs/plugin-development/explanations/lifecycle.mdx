...
Both `setAction` and `setInlineAction` task actions can optionally return a `Result` value. If the action returns a result with `success: false`, the CLI sets the process exit code to 1.
