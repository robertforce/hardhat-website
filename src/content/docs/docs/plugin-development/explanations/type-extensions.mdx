...
Your `type-extensions.ts` file should follow these rules for `export type *` to work correctly:

- **No side effects**: the file should not execute any runtime code.
- **No exported types**: do not export any types, only use `declare module` augmentations.
- **Use `export {};` as the module marker**: as the file has no imports, TypeScript treats it as a script rather than a module by default. Adding `export {};` forces module mode without introducing a runtime import. Alternatively, set [`"moduleDetection": "force"`](https://www.typescriptlang.org/tsconfig/#moduleDetection) in your `tsconfig.json`.
