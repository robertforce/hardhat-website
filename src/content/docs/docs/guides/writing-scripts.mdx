...
- `HARDHAT_VERBOSITY`: Sets the verbosity level for call traces

To view the full list of Global Options available in Hardhat, run:

<Run command="hardhat --help" />

## Debugging scripts with call traces

When your script connects to a simulated network and involves transactions, you can use the `--verbosity` (or `-v`) flag to show call traces. For example, `-vvv` shows traces for failing transactions:
...
Call traces show the sequence of contract function calls, making it easier to debug deployment scripts and understand complex interactions. See [Global Options](/docs/explanations/global-options#verbosity) for the full list of verbosity levels.
