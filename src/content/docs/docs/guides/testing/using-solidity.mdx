...
To run only the tests whose name matches a string or regular expression, use the `--grep` option:

<Run command="hardhat test solidity --grep testInc" />

To skip the matching tests instead, use `--grep-exclude`:

<Run command="hardhat test solidity --grep-exclude testIncBy" />

Combine both options to run the tests that match `--grep` while skipping any that also match `--grep-exclude`.
...
To define several sets of settings and choose between them when running your tests, read the [Test Profiles](/docs/guides/testing/test-profiles) guide.
