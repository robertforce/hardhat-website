...
To keep this larger bytecode and higher gas usage from tripping size and gas limits, `--coverage` adjusts some defaults. In every case an explicit value in your config is respected — the override only applies when you haven't set the option yourself:

- The `allowUnlimitedContractSize` [setting of `edr-simulated` networks](/docs/reference/configuration#simulated-network-options) defaults to `true`.
- The `blockGasLimit` and `transactionGasCap` settings of [`edr-simulated` networks](/docs/reference/configuration#simulated-network-options) and [Solidity tests](/docs/reference/configuration#solidity-tests-configuration) default to disabled.
