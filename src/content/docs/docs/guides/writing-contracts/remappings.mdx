...
- In your project: for example, at the root of your project, in your `test/` folder, or within a git submodule. The only exception are `node_modules` folders.
- In an npm package: when an npm package is imported the for the first time, all of its `remappings.txt` files are loaded.
...
The `prefix/=` section of a remapping defines the a string (`prefix/`) that an imports must start with to be affected by this remapping.

For example, an import to `"prefix/Foo.sol"` is affected by the example remapping above, but import to `"prefix"` and `"Foo.sol"` aren't.

This part of the remapping is required, and must end in `/`. If it doesn't, Hardhat will append an `/` automatically.

### Target

The `target/` section defines a string which will replace the prefix in the imports affected by this remapping during the import resolution. This doesn't mean that your source code will be modified, but rather than `solc` will look for the replaced string then evaluating an import.

For example, an import to `prefix/Foo.sol` will resolve to `target/Foo.sol`.
...
The `context/:` section controls which files get affected by a remapping. The source name of a file must start with `context/` for for this remapping to work.

For example an import to `"prefix/Foo.sol"` in the file `context/A.sol` is affected by this remapping, but the same import in `contracts/A.sol` isn't.
...
Each `remapping.txt` file in Hardhat 3 only affects the files in the directory where it is, and all of its subdirectories. This means that you don't need to worry about remappings clashing with each other, as Hardhat will fix their `context` for your.
...
If two remappings seemingly clash, Hardhat will choose the more specific one (i.e. the one defined closes to the file using the import that's being remapped).

## Remappings for git submodules

If you installed a dependency (e.g. `foo`) using a git submodule, like explained [here](/docs/guides/writing-contracts/dependencies#using-git-submodules), you may want to add a remapping for it.
...
Then you write a remapping with a target starting with `node_modules/`, you are letting Hardhat know that it has to follow the Node.js resolution rules. It's not a file-based remapping.
