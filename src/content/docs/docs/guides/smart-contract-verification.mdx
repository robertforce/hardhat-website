...
You can get an Etherscan API key by following [this guide](https://etherscan.io/myapikey). This plugin uses Etherscan's API v2, so it only requires a single API key, which works with every supported chain.
