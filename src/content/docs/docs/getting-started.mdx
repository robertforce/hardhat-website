...
- [Node.js](https://nodejs.org/) v22.13.0 or later (Docker users: pin a specific tag such as `node:22.13-alpine` — `node:22-alpine` may ship an older patch)
...
You can also initialize a project in non-interactive mode, which is useful for automation scripts and AI agents.

To create the same project as above without prompts, run:

<RunRemote command="hardhat --init --template node-test-runner-viem" />

The `--template` option specifies which template to use. To see all available templates, run:

<RunRemote command="hardhat --init --templates" />
