...
This cheatcode works similar to [`getCode`](/docs/cheatcodes/external/get-code) but only returns the **deployed** bytecode (aka runtime
...
:::note
`getDeployedCode` requires read permission for the output directory, see [file cheatcodes](/docs/cheatcodes/file/fs).
...
Deploy a stateless contract at an arbitrary address using `getDeployedCode` and [`etch`](/docs/cheatcodes/environment/etch).
...
[`getCode`](/docs/cheatcodes/external/get-code)
[`etch`](/docs/cheatcodes/environment/etch)
