...
This cheatcode works similar to [`getCode`](/docs/cheatcodes/get-code) but only returns the **deployed** bytecode (aka runtime
...
> ℹ️ **Note**
>
> `getDeployedCode` requires read permission for the output directory, see [file cheatcodes](/docs/cheatcodes/fs).
>
...
Deploy a stateless contract at an arbitrary address using `getDeployedCode` and [`etch`](/docs/cheatcodes/etch).
...
[`getCode`](/docs/cheatcodes/get-code)
