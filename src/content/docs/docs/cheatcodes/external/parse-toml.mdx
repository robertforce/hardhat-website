...
If you receive this error, make sure that you enable read permissions in [Solidity tests configuration](https://hardhat.org/docs/reference/configuration#solidity-tests-configuration) using the [`fsPermissions` object](/docs/cheatcodes/fs).

### References

- Helper Library: [stdToml.sol](https://github.com/foundry-rs/forge-std/blob/master/src/StdToml.sol)
- [File Cheatcodes](/docs/cheatcodes/fs): cheatcodes for working with files
