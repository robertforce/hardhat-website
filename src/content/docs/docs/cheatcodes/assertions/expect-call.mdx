---
title: expectCall
description: expectCall cheatcode documentation
---
...
```solidity
function expectCall(address callee, uint256 value, uint64 gas, bytes calldata data) external;
```

```solidity
function expectCall(address callee, uint256 value, uint64 gas, bytes calldata data, uint64 count) external;
```

### Description

Expects a call to a specified address `callee`, where the call data either strictly or loosely matches `data`. If `gas` is specified, it is also checked.
