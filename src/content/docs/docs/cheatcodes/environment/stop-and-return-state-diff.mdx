...
Retrieves state changes recorded after a call to [`startStateDiffRecording`](/docs/cheatcodes/start-state-diff-recording). This function will consume the recorded state diffs when called and disable state diff recording. One may call `startStateDiffRecording` to resume recording.
