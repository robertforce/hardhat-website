...
description: mockCallRevert cheatcode documentation
---
...
function mockCallRevert(address where, bytes4 data, bytes calldata retdata) external;
```
...
Reverted mock calls are in effect until [`clearMockedCalls`](/docs/cheatcodes/clear-mocked-calls) is called.
