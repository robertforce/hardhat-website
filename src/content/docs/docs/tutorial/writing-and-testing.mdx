...
Solidity tests are just regular Solidity files. Hardhat interprets any file ending with `.t.sol`, or any Solidity file inside the test directory, as a test file. Each public function whose name starts with `test` is executed as a test. If it finishes without reverting, the test passes. If it reverts, the test fails.

Let's run all the tests in your project like this:
...
And run the tests again. This time, we'll use a different command, that only runs Solidity tests.
