...
This module deploys an instance of `Counter` and calls `incBy(5)` once it's deployed. These functions don't execute the deployment transactions or function call immediately. They describe what should be done. Ignition then executes the deployment steps in the right order, handling dependencies, recovering from errors, and ensuring everything runs correctly.
...
Ignition remembers the deployment state. If you run the deployment again, nothing will happen the second time because the module was already executed. Try rerunning the previous command to see this.
