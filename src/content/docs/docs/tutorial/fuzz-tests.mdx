...
Hardhat 3 supports fuzz tests in Solidity. These tests run the same function many times with random inputs, which Hardhat automatically generates. To create one, you only need to write a test function that takes parameters.

Let's add a fuzz test to our `Counter.t.sol` file:

```solidity collapse={1-5,10-33} ins={35-41}
...
When you run these tests, the `testFuzz_Inc` function will be called multiple times with values that fit in a `uint8` (between 0 and 255). This tests the `inc` function with a wide range of inputs.

Run your tests again and check that everything passes:
