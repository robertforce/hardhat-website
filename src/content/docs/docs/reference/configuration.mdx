...
- `gasPrice`: Its value should be `"auto"` or a number or bigint. If `"auto"` is used, the gas price will be automatically determined — using EIP-1559 fee history when available, or falling back to a legacy `eth_gasPrice` call otherwise. If a number or bigint is used, it will be used as a fixed legacy gas price for every transaction. Explicit gas price values in individual transactions always override this setting. Default value: `"auto"`.
