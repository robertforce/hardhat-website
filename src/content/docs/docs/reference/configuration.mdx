...
- An object which describes the configuration for a single compiler. It contains the following keys:
  ...
  - `splitTestsCompilation`: A boolean that controls if the Solidity tests should be built independently from the contracts. Defaults to `false`.
...
- An object which describes multiple compilers and their respective configurations. It contains the following:
  ...
  - `splitTestsCompilation`: See above.
  - `toolVersionsInBuildInfo`: If `true`, the version of Hardhat is included in Build Info files. Defaults to `true` for the `production` Build Profile and `false` for all other profiles.
- An object which describes multiple build profiles and their respective configurations. It contains the following:
  - `profiles`: A map of Build Profile names to compiler configuration objects like either of the two above, except for `splitTestsCompilation`.
  - `npmFilesToBuild`: See above.
...
There are two kinds of networks in Hardhat: in-memory simulated networks and [JSON-RPC](https://ethereum.org/developers/docs/apis/json-rpc/) based networks. You can use either of these by setting the `type` field in the network configuration to either `edr-simulated` or `http`, respectively.
...
- `gas`: Its value should be `"auto"` or a number or bigint. If a number or bigint is used, it will be the gas limit used by default in every transaction. If `"auto"` is used, the gas limit will be automatically estimated via `eth_estimateGas` before each transaction. This estimation can be adjusted with the `gasMultiplier` option. Explicit gas limits in individual transactions always override this setting. Default value: `"auto"`.
- `gasMultiplier`: A number used to multiply the results of gas estimation to give it some slack due to the uncertainty of the estimation process. Only applies when `gas` is `"auto"`. Default value: `1`.
- `gasPrice`: Its value should be `"auto"` or a number or bigint. If `"auto"` is used, Hardhat populates the transaction's fee fields automatically: on EIP-1559 networks it sets `maxFeePerGas` and `maxPriorityFeePerGas` using `eth_feeHistory`; on non-EIP-1559 networks it sets a legacy `gasPrice` using `eth_gasPrice`. If a number or bigint is used, it is set as a fixed legacy `gasPrice` on every transaction. Explicit fee fields in individual transactions always override this setting. Default value: `"auto"`.

:::tip
These gas options only affect transactions sent via `eth_sendTransaction`. They do not modify the results of direct `eth_estimateGas` or `eth_gasPrice` RPC calls. Both the `hardhat-ethers` and `hardhat-viem` plugins respect these settings.
:::
...
- `blockGasLimit`: The block gas limit to use in Hardhat Network's blockchain. Set it to `false` to disable the block gas limit. Default value: `60_000_000`.
- `transactionGasCap`: The maximum amount of gas a single transaction may use. Set it to `false` to disable the cap. Default value: the [EIP-7825](https://eips.ethereum.org/EIPS/eip-7825) transaction gas cap (`16_777_216`) on the `osaka` hardfork and later, and the block gas limit on earlier hardforks.
...
- `gasEstimationMode`: Controls how `eth_estimateGas` computes its result. When set to `"noInternalOutOfGas"`, the returned estimate also prevents calls internal to the transaction from running out of gas, and the estimation fails if an internal call runs out of gas no matter how much gas is provided. When set to `"topLevelSuccess"`, the estimate only needs to make the top-level call succeed, even if an internal call runs out of gas along the way. See [gas estimation modes](/docs/reference/edr-simulated-networks#gas-estimation-modes) for more details. Default value: `"noInternalOutOfGas"`.
- `hardfork`: This setting changes how the simulated network works, to mimic the target chain at a given hardfork. The available values depend on the `chainType`:
  - For `l1` and `generic` chain types: `"byzantium"`, `"constantinople"`, `"petersburg"`, `"istanbul"`, `"muirGlacier"`, `"berlin"`, `"london"`, `"arrowGlacier"`, `"grayGlacier"`, `"merge"`, `"shanghai"`, `"cancun"`, `"prague"`, `"osaka"` and `"amsterdam"` (experimental, see [Amsterdam support](/docs/reference/amsterdam-support) for details). Default value: `"osaka"`.
  - For the `op` chain type: `"bedrock"`, `"regolith"`, `"canyon"`, `"ecotone"`, `"fjord"`, `"granite"`, `"holocene"`, and `"isthmus"`. Default value: `"isthmus"`.
...
- `gasLimit`: The gas limit for each test case. Defaults to `9_223_372_036_854_775_807` (`i64::MAX`).
...
- `blockGasLimit`: The `block.gaslimit` value during EVM execution. Set it to a number or bigint to enforce a limit, or to `false` to disable it. Disabled by default.
- `transactionGasCap`: The maximum amount of gas a single transaction may use. Set it to a number or bigint to enforce a cap, or to `false` to disable it. Disabled by default.
...
Instead of the options above, `test.solidity` can contain a single `profiles` entry: a map of [Test Profile](/docs/guides/testing/test-profiles) names to objects with those same options.
...
The two shapes are mutually exclusive: a config with `profiles` can't also set options at the top level. Setting them at the top level is equivalent to defining only a `default` profile.
