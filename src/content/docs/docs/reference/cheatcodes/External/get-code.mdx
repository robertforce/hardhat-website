...
:::note
`getCode` requires read permission for the output directory, see [file cheatcodes](/docs/cheatcodes/file/fs).
...
Deploy a contract to an arbitrary address by combining `getCode` and [`etch`](/docs/cheatcodes/environment/etch)
...
[`getDeployedCode`](/docs/cheatcodes/external/get-deployed-code)
