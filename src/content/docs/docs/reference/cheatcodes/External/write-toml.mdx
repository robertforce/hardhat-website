---
title: writeToml
description: writeToml cheatcode documentation
---
...
This is usually built through [serializeJson](/docs/reference/cheatcodes/external/serialize-json).
...
**Remember:** The file path `path` needs to be in the allowed paths. Read more in [File cheatcodes](/docs/reference/cheatcodes/file/fs).

#### JSON Paths

The `valueKey` parameter uses JSONPath syntax to specify where in the JSON structure to place or update the value. We use the [jsonpath_lib](https://docs.rs/jsonpath_lib/latest/jsonpath_lib/) library internally to parse these paths.
...
- [serializeJson](/docs/reference/cheatcodes/external/serialize-json)
