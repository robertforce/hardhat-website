---
title: expectEmit
description: expectEmit cheatcode documentation
---
...
1. Call the cheatcode, specifying whether we should check the first, second or third topic, and the log data (`expectEmit()` checks them all). Topic 0 is always checked.
...
If `count` is used, expect a the given number of logs with the provided parameters.
