---
title: expectRevert
description: expectRevert cheatcode documentation
---
...
By default, `expectRevert*` cheatcodes work only for calls with greater depth than test depth.
Expecting reverts at the same depth as test depth can be enabled by setting `allowInternalExpectRevert` to `true` in the [Solidity tests configuration](https://hardhat.org/docs/reference/configuration#solidity-tests-configuration).
:::
...
This means, for example, we can call [`prank`](/docs/reference/cheatcodes/environment/prank) immediately before the reverting call.
...
[Std Errors](https://getfoundry.sh/reference/forge-std/std-errors)
