---
title: stopAndReturnStateDiff
description: stopAndReturnStateDiff cheatcode documentation
---
...
function stopAndReturnStateDiff()
  external
  returns (AccountAccess[] memory accesses);
```

### Description

Retrieves state changes recorded after a call to [`startStateDiffRecording`](/docs/reference/cheatcodes/environment/start-state-diff-recording). This function will consume the recorded state diffs when called and disable state diff recording. One may call `startStateDiffRecording` to resume recording.
