...
Foundry's `[profile.<name>]` test settings map to Hardhat's [Test Profiles](/docs/guides/testing/test-profiles), and its `forge-config:` annotations to Hardhat's [inline configuration](/docs/guides/testing/inline-configuration). Both differ from Foundry in ways that matter when migrating, so read the [Foundry migration guide](/docs/migrate-from-foundry) first.
