...
When a `NetworkConnection` is created, Hardhat sets up a chain of request handlers based on the [network configuration](/docs/reference/configuration#shared-network-options). For `eth_sendTransaction` requests, these handlers automatically populate the `gas` and `gasPrice` fields when they are missing from the transaction:

1. **Gas price handler** — If `gasPrice` is `"auto"`, an EIP-1559 fee suggestion is computed (using `eth_feeHistory`), falling back to `eth_gasPrice` for non-EIP-1559 networks. If `gasPrice` is a fixed value, it is set as a legacy `gasPrice`.
