...
When a `NetworkConnection` is created, Hardhat sets up a chain of request handlers based on the [network configuration](/docs/reference/configuration#shared-network-options). For `eth_sendTransaction` requests, these handlers automatically populate the transaction's gas limit and fee fields when they are missing:

1. **Fee handler** — If `gasPrice` is `"auto"`, Hardhat sets `maxFeePerGas` and `maxPriorityFeePerGas` (computed from `eth_feeHistory`) on EIP-1559 networks, or falls back to a legacy `gasPrice` (from `eth_gasPrice`) otherwise. If `gasPrice` is a fixed value, it is set as a legacy `gasPrice`.
2. **Gas handler** — If `gas` is `"auto"`, the gas limit is estimated via `eth_estimateGas` and multiplied by `gasMultiplier`. If `gas` is a fixed value, it is used directly.

These handlers only modify `eth_sendTransaction` requests. Other RPC calls like `eth_estimateGas` and `eth_gasPrice` are not affected. When a transaction already includes explicit gas values, the handlers leave them as-is.

This works transparently with both the `hardhat-ethers` and `hardhat-viem` plugins.
