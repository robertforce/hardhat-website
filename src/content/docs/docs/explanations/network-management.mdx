...
- If the Network Config is of `type` `"http"`, the JSON-RPC server in the `url` setting will be used. No synchronization between two HTTP Network Connections is performed by Hardhat. Neither at the HTTP layer, nor protocol layer (e.g. when handling account nonces).
