...
Hardhat uses EDR to compute code coverage for your tests by instrumenting your contracts and tracking which lines are executed. Simply run your tests with the `--coverage` flag. An HTML report is also automatically generated at `/coverage/html/index.html`.
