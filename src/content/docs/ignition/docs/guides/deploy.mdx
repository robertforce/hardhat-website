...
<Run command="hardhat ignition deploy ignition/modules/MyModule.ts" />
...
<Run command="hardhat ignition deploy ignition/modules/Apollo.ts --parameters ignition/parameters.json" />
...
<Run command="hardhat ignition deployments" />

To check on the current status of a deployment, run:

<Run command="hardhat ignition status DeploymentId" />
